...
\title{Atom-CAD: A Rule-Based Atomic Token Representation\\for Converting STEP B-Rep Models to Structured CAD Sequences}
...
We evaluate Atom Language on 29 STEP models organized into two test sets. \textbf{Exp1} contains 18 single-feature models, each exercising one of 18 SolidWorks operation types: ReferencePlane, ReferenceAxis, ReferencePoint, Extrude, Cut, Hole, Revolve, RevolveCut, Loft, Sweep, Fillet, Chamfer, Draft, Mirror, PatternLin, PatternCir, Shell, and Rib. \textbf{Exp2} contains 11 complex multi-feature models: five from the MSF dataset~\cite{shi2020brep} and six generated via SolidWorks macros combining patterns, mirrors, ribs, and shells.
...
Table~\ref{tab:exp1} reports the results for all 18 operation types. Every operation is successfully recognized and serialized into a valid atom sequence, achieving \textbf{100\% coverage}. The average token count per model is 382, with Direct operations (Shell, Rib) producing the longest sequences (avg.\ 722 tokens) due to their complex face topology.
...
\begin{tabular}{llcr}
\toprule
Category & Operations & Count & Avg Tokens \\
\midrule
Reference/Datum & Ref.Plane, Ref.Axis, Ref.Point & 3 & 189 \\
Sketch-Driven & Extrude, Cut, Hole, Revolve, RevolveCut & 5 & 462 \\
Freeform & Loft, Sweep & 2 & 175 \\
Finishing & Fillet, Chamfer, Draft & 3 & 509 \\
Pattern/Symmetry & Mirror, PatternLin, PatternCir & 3 & 227 \\
Direct & Shell, Rib & 2 & 722 \\
\midrule
\textbf{All 18 operations} & & \textbf{18/18} & \textbf{382} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:exp2} shows results for 11 complex models. All models are successfully decomposed into valid workstep sequences. The most complex model (SW-Rib) produces 802 tokens with 12 features, while SW-CircPattern achieves the highest part count (6 parts, 18 features).
...
\midrule
MSF-00018936 & 1 & 1 & \checkmark & 151 \\
MSF-00018939 & 1 & 1 & \checkmark & 137 \\
MSF-00016570 & 1 & 4 & \checkmark & 273 \\
MSF-00018727 & 1 & 4 & \checkmark & 273 \\
MSF-00016571 & 1 & 4 & \checkmark & 273 \\
SW-CircPattern & 6 & 18 & \checkmark & 150 \\
SW-Mirror & 2 & 12 & \checkmark & 388 \\
SW-Rib & 1 & 12 & \checkmark & 802 \\
SW-Shell & 1 & 9 & \checkmark & 641 \\
SW-LinPattern & 4 & 12 & \checkmark & 142 \\
Synthetic & 6 & 18 & \checkmark & 150 \\
\midrule
...
To validate that atom sequences faithfully represent the original geometry, we implement a closed-loop verification pipeline: atom sequence $\to$ CAD-order commands $\to$ PythonOCC execution $\to$ reconstructed STEP file. We then compare the reconstructed solid against the original using point cloud sampling (8{,}192 points) and Chamfer Distance (CD).

All 20 models (18 single-feature + 2 MSF) were verified end-to-end. Of these, 15 produce valid reconstructed solids with finite CD. The mean CD is 0.098 with a median of 0.103, confirming geometric fidelity of the atom representation. The best reconstruction (ReferenceAxis) achieves CD\,=\,0.001, while the most challenging (LinearPattern) reaches CD\,=\,0.217. Five models yield infinite CD due to zero-volume reconstructions in reference-only or sweep operations where the OCC executor lacks sufficient sketch context. Table~\ref{tab:occ} summarizes representative results.

\begin{table}[t]
\caption{Closed-loop OCC verification: selected models with finite Chamfer Distance.}
\label{tab:occ}
\centering
\small
\begin{tabular}{lrrrr}
\toprule
Model & CD$_{L1}$ & HD & Vol.\ Ratio & Valid \\
\midrule
ReferenceAxis & 0.001 & 100.00 & 1.000 & \checkmark \\
RevolveBoss & 0.029 & 10.02 & 2.139 & -- \\
Draft & 0.033 & 30.24 & 1.783 & -- \\
Loft & 0.044 & 5.78 & 0.818 & \checkmark \\
MSF-00018936 & 0.073 & 9.89 & 0.000 & -- \\
ExtrudeBoss & 0.080 & 20.03 & 0.200 & \checkmark \\
RevolveCut & 0.103 & 77.51 & 1.580 & \checkmark \\
MSF-00018939 & 0.104 & 9.88 & 0.000 & \checkmark \\
Mirror & 0.144 & 30.01 & 0.125 & \checkmark \\
Fillet & 0.155 & 32.96 & 0.169 & \checkmark \\
Chamfer & 0.174 & 38.39 & 0.032 & \checkmark \\
LinearPattern & 0.217 & 30.01 & 0.125 & \checkmark \\
\midrule
\textbf{Mean (15 finite)} & \textbf{0.098} & & & \textbf{15/20} \\
\bottomrule
\end{tabular}
\end{table}
...
Metric & Value \\
\midrule
Operation coverage (Exp1) & 18/18 (100\%) \\
Complex model decomposition (Exp2) & 11/11 (100\%) \\
OCC closed-loop finite CD & 15/20 (75\%) \\
Mean Chamfer Distance (finite) & 0.098 \\
Median Chamfer Distance & 0.103 \\
Avg tokens per model (Exp1) & 382 \\
Cross-stage validation & 29/29 \\
OCC step success rate & 99.2\% \\
\bottomrule
\end{tabular}
\end{table}
...
The pipeline processes each model in under 15\,ms on a single CPU core. Table~\ref{tab:timing} shows the per-stage breakdown averaged over all 29 models.

\begin{table}[t]
\caption{Average per-model runtime breakdown (29 models).}
\label{tab:timing}
\centering
\begin{tabular}{lr}
\toprule
Stage & Time (ms) \\
\midrule
Face graph extraction & 11.8 \\
Part partition & $<$0.1 \\
Feature recognition & 0.5 \\
Workstep construction & 0.3 \\
Atom serialization & 0.4 \\
\midrule
...
\subsection{Diffusion-Based Operation Prediction}

To demonstrate that Atom Language sequences can serve as structured training targets for learning-based approaches, we train a 3D U-Net diffusion model (Operation-as-Noise, Scheme~1) that predicts the CAD operation type, parameters, and affected region from a noised SDF volume. The model treats the manufacturing sequence as a denoising process: given a final-state SDF corrupted at timestep $t$, it predicts which operation was applied.

We conduct a design-space exploration (DSE) over 6 configurations on 8$\times$A100 GPUs (6 cards in parallel), varying base channels (32/48), learning rate ($10^{-4}$/$3\times10^{-4}$), and dropout (0.1/0.2). Table~\ref{tab:diffusion} reports the evaluation results on 256 synthetic test samples covering 9 operation classes.

\begin{table}[t]
\caption{Diffusion model evaluation: per-class type accuracy and region IoU on 256 test samples (best checkpoint, base\_ch=32, 30 epochs).}
\label{tab:diffusion}
\centering
\small
\begin{tabular}{lrrr}
\toprule
Operation & Accuracy & Region IoU & Param MAE \\
\midrule
Extrude Boss & 0.804 & 0.451 & 0.139 \\
Extrude Cut & 0.566 & 0.142 & 0.127 \\
Revolve Boss & 0.481 & 0.379 & 0.165 \\
Shell & 0.600 & 0.481 & 0.163 \\
Hole & 0.051 & 0.124 & 0.152 \\
Revolve Cut & 0.000 & 0.181 & 0.157 \\
Fillet & 0.000 & 0.000 & 0.124 \\
Chamfer & 0.000 & 0.000 & 0.132 \\
Mirror & 0.000 & 0.087 & 0.144 \\
\midrule
\textbf{Overall} & \textbf{0.387} & \textbf{0.205} & \textbf{0.145} \\
\bottomrule
\end{tabular}
\end{table}

The model achieves 38.7\% overall type accuracy, with strong performance on volumetric operations (Extrude Boss 80.4\%, Shell 60.0\%, Extrude Cut 56.6\%) where the SDF signature is distinctive. Surface-modifying operations (Fillet, Chamfer) remain challenging as they produce subtle SDF changes. These results establish a baseline for learning inverse CAD sequences from Atom Language representations and motivate future work on larger training sets and multi-step sequential prediction.

\subsection{Limitations}

Our evaluation has several limitations. First, the test set of 29 models, while covering all 18 operation types, is relatively small; generalization to industrial-scale datasets with hundreds of features per model remains to be validated. Second, the OCC closed-loop verification achieves finite CD on 15 out of 20 models; the remaining 5 fail due to reference-only or sweep operations where the executor lacks sufficient sketch context. Third, the rule-based approach may not generalize to novel feature types not covered by the current rule set. Fourth, the diffusion model evaluation uses synthetic SDF data; real STEP-derived SDF volumes may exhibit different characteristics. Finally, the current pipeline assumes manifold B-Rep solids and may fail on non-manifold or degenerate geometry.
...
The proposed representation covers 19 workstep types with over 80 atomic tokens across nine semantic categories, supporting the complete spectrum of subtractive manufacturing operations. Experiments on 29 models demonstrate 100\% feature coverage, successful closed-loop geometric verification on 15/20 models with a mean Chamfer Distance of 0.098, and an average pipeline runtime of 13\,ms per model. A complementary diffusion-based operation predictor achieves 38.7\% type accuracy on 9-class synthetic data, establishing a baseline for learning inverse CAD sequences from the proposed representation.

Future work includes: (1) scaling evaluation to 100+ industrial models for generalization validation, (2) integrating learned feature recognition (GNN/Transformer) alongside rule-based methods to handle novel feature types, (3) extending the token vocabulary to cover sheet metal and additive manufacturing operations, (4) improving the diffusion model with real STEP-derived SDF data and multi-step sequential prediction, and (5) using atom sequences as structured training targets for next-generation generative CAD models.
