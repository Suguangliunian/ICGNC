...
Experiments on 29 models spanning 18 SolidWorks operations demonstrate that Atom Language achieves 100\% single-feature coverage, 100\% complex-model decomposition success, and a mean reconstruction Chamfer Distance of 0.098 (15/20 models with finite CD).
...
Table~\ref{tab:exp2} shows results for 11 complex models. All models are successfully decomposed into valid workstep sequences. The most complex model (SW-Rib) produces 802 tokens with 12 recognized features.
...
To validate that atom sequences faithfully represent the original geometry, we implement a closed-loop verification pipeline: atom sequence $\to$ CAD-order commands $\to$ PythonOCC execution $\to$ reconstructed STEP file. We then compare the reconstructed solid against the original using point cloud sampling (8{,}192 points) and normalized Chamfer Distance (CD$_{L1}$).

Table~\ref{tab:occ} reports the closed-loop verification results on 20 models (18 single-feature + 2 MSF complex). Of 19 successfully rebuilt solids, 15 yield finite CD values. The mean CD$_{L1}$ is 0.098 with a median of 0.103, confirming geometric fidelity of the atom representation. The best reconstruction (ReferenceAxis) achieves CD$_{L1} = 0.001$, indicating near-perfect geometric recovery for simple rotational features.

\begin{table}[t]
\caption{Closed-loop OCC verification: top-10 models by CD$_{L1}$.}
\label{tab:occ}
\centering
\small
\begin{tabular}{lrrr}
\toprule
Model & CD$_{L1}$ & Vol. Ratio & Status \\
\midrule
ReferenceAxis & 0.001 & 1.000 & \checkmark \\
RevolveBoss & 0.028 & 2.139 & \checkmark \\
Shell & 0.030 & 3.356 & \checkmark \\
Draft & 0.033 & 1.783 & \checkmark \\
Loft & 0.044 & 0.818 & \checkmark \\
MSF-00018936 & 0.073 & --- & \checkmark \\
ExtrudeBoss & 0.080 & 0.200 & \checkmark \\
RevolveCut & 0.103 & 1.580 & \checkmark \\
MSF-00018939 & 0.104 & --- & \checkmark \\
CircPattern & 0.142 & 0.333 & \checkmark \\
\midrule
\multicolumn{2}{l}{\textbf{Mean (15 finite)}} & \multicolumn{2}{r}{\textbf{CD$_{L1}$ = 0.098}} \\
\bottomrule
\end{tabular}
\end{table}
...
The pipeline processes each model in under 15\,ms on a single CPU core (Intel Xeon, 2.4\,GHz). Table~\ref{tab:timing} shows the per-stage breakdown averaged over all 29 test models.
